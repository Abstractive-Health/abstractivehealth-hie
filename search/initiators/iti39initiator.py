...
class ITI39Initiator:
    ...
    async def send_request(self):
        try:
            ...
            DocumentRequest = [
                {
                    "HomeCommunityId": "urn:oid:" + pair['replacement_hcid'],
            ...
            saml = Saml()
            use_sha256 = True if "hospitalconnect" in self.responder_url else False
            saml_assertion, refId = saml.create_saml_assertion_string(
                "http://ihe.connectathon.XUA/X-ServiceProvider-IHE-Connectathon", "MedicalDoctor",
                "TREATMENT", self.user_qualifications, use_sha256=use_sha256)
            ...
            try:
                async with self.async_session.post(endpoint, data=signed_message, headers=headers) as response:
                    ...
                    self.request_latency = (datetime.now() - start_time).total_seconds()
                ...
                print("timeout or connection error with endpoint", self.responder_url)
    ...
    def process_response(self):
        # Process the XCPD response here
        # You can extract patient information from the response
        s3_write_client = boto3.client('s3', endpoint_url="https://s3.amazonaws.com/")
        s3_write_client.put_object(Body=str(self.response_xml),
                                   Bucket=f"",
                                   Key="39response/" + str(uuid.uuid4()) + ".xml")
        return self.response_xml
