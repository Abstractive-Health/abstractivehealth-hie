...
class ITI55Initiator:
    ...
    async def send_request(self):
        try:
            ...
            s3_write_client.put_object(Body=str(signed_message),
                                       Bucket=f"", Key="55request/" +
                                       str(self.current_time) + "_" + str(self.receiver_hcid) +
                                       "_" + str(uuid.uuid4()) + ".xml")
            ...
            try:
                async with self.async_session.post(endpoint, data=signed_message, headers=headers) as response:
                    ...
                    self.request_latency = (datetime.now() - request_start).total_seconds()
                    self.response_xml = response_text
                    self.process_response()

            except (aiohttp.ClientConnectionError, aiohttp.ClientResponseError) as e:
                print(e, " error with endpoint", self.responder_url, "retrying once")
                try:
                    async with self.async_session.post(endpoint, data=signed_message, headers=headers) as response:
                        response_text = await response.text()
                        print(f"got response from Endpoint, {endpoint}")
                    ...
                    print(e, " still error with endpoint", self.responder_url, "done retrying")
                    self.response_xml = None

            except asyncio.exceptions.TimeoutError as e:
                print(f"asyncio timed out, {endpoint}")
                self.response_xml = None
    ...
    def process_response(self):
        ...
        #                            Bucket=f"",
        #                            Key="55response/" + str(self.current_time)
        #                            + "_" + str(self.receiver_hcid)
        #                            + "_" + str(uuid.uuid4()) + ".xml")
        return self.response_xml
