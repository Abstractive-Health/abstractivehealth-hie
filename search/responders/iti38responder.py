...
class ITI38Responder:
    def __init__(self, request, initiator_url=None):
        ...
        # hardcode values
        self.doc_name = "Hospital and Outpatient Visits Narrative"
        self.loinc_code = ""
        self.format_code = ""
        self.format_system = ""
        self.hcf = ""
    ...
    def write_request(self):
        ...
        s3_write_client.put_object(Body=etree.tostring(self.request),
                                   Bucket=f"",
                                   Key="38request/" + str(uuid.uuid4()) + ".xml")
    ...
    def process_xca_find_documents_request(self):
        ...
        # # check hcid is intended for us
        # intended_hcid = root.find('.//{*}AdhocQuery').attrib['home']
        # if intended_hcid != "urn:oid:"+self.hcid:
        #     raise Exception(f"request is not to us, it's to {intended_hcid}")
        ...
        patient_ids, org_hcids = [], set()
        for element in patient_id_elements:
            patient_id, org_hcid = eval(element.text).split('^^^')
            org_hcid = org_hcid.replace('&', '').replace('ISO', '')
            patient_ids.append(patient_id)
            org_hcids.add(org_hcid)

        self.patient_ids = patient_ids
        # TODO: currently only supporting one org_hcid; change to support multiple in the future
        self.org_hcid = None
        if len(org_hcids) == 1:
            org_hcid = org_hcids.pop()
            if utils.is_valid_org_hcid(org_hcid):
                self.org_hcid = org_hcid
    ...
    def search_db_for_documents_metadata(self):
        ...
        '''
        # TODO: switch to signed notes from signed notes table
        hcid = rid = self.org_hcid
        results = set()
        for pid in self.patient_ids:
            ...
            response_body = json.loads(response["Payload"].read().decode("utf-8"))["body"]
            doc_ids = json.loads(response_body)
            print("doc_ids", doc_ids)

            for doc_id in doc_ids:
                results.add((hcid, rid, doc_id, pid, self.doc_name, self.loinc_code,
                            self.format_code, self.format_system, self.hcf, self.hcf_system))
    ...
    def generate_adhoc_query_response(self):
        '''
        ...
        if self.returntype == "ObjectRef":
            ...
            RegistryPackage.set('home', "urn:oid:" + self.org_hcid)
    ...
    def generate_response_body(self):
        ...
        self.generate_adhoc_query_response()
        return self.response_body
