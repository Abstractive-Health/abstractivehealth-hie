...
secret_id = os.environ.get("ENV") + "-lambda-hie-patient"
secret_params = json.loads(secretsmanager.get_secret_value(SecretId=secret_id)["SecretString"])
cq_cert = secret_params['carequality_certificate']
cq_private_key = secret_params['cqkey_https']
...
class Saml(object):
    ...
    def create_saml_assertion_string(
            self, audience, role, purposeOfUse, user_qualifications, use_sha256=False):
        ...
        # # # Create the PurposeOfUse element with the default namespace
        # pou_element = etree.Element("{urn:hl7-org:v3}PurposeOfUse", nsmap=nsmap)

        # # Add attributes to the PurposeOfUse element
        # pou_element.set("{http://www.w3.org/2001/XMLSchema-instance}type", "CE")
        # pou_element.set("code", purposeOfUse)
        # pou_element.set("codeSystem", "2.16.840.1.113883.3.18.7.1")
        # pou_element.set("codeSystemName", "nhin-purpose")
        # pou_element.set("displayName", purposeOfUse)

        # role_element = etree.Element("{urn:hl7-org:v3}Role", nsmap=nsmap)

        # # Add attributes to the Role element
        # role_element.set("{http://www.w3.org/2001/XMLSchema-instance}type", "CE")
        # role_element.set("code", "112247003")
        # role_element.set("codeSystem", "2.16.840.1.113883.6.96")
        # role_element.set("codeSystemName", "SNOMED_CT")
        # role_element.set("displayName", "Medical doctor")
        ...
        pou_element = p_E.PurposeOfUse(
            {QName("http://www.w3.org/2001/XMLSchema-instance", "type"): "ns2:CE"},
            code=purposeOfUse,
        ...
        if use_sha256:
            signed_saml_root = XMLSigner(
                ...
                digest_algorithm=DigestAlgorithm.SHA256).sign(
    ...
    def sign_soap_message(self, soap_message, refID, own_url, destination_url, use_sha256=False):
        ...
        soap_message = soap_message.decode()
        soap_message = soap_message.replace(
            "<samlns:AttributeValue xsi:type=\"xs:string\"",
            "<samlns:AttributeValue xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" xsi:type=\"xs:string\"")
        soap_message = soap_message.replace(":ns0", "")
        soap_message = soap_message.replace("ns0:", "")
    ...
    #     replyElement = etree.fromstring(
    #         '''<a:ReplyTo xmlns:a="http://www.w3.org/2005/08/addressing/anonymous">
    # <a:Address>'''+own_url+'''</a:Address>
    # </a:ReplyTo>''')
    #     toElement = etree.fromstring('<a:To soap-env:mustUnderstand="1" xmlns:a="http://www.w3.org/2005/08/addressing" xmlns:soap-env="http://www.w3.org/2003/05/soap-envelope" xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd" wsu:Id="_0"></a:To>')

    #     header.append(replyElement)
        ...
        if use_sha256:
            signedXml = XMLSigner(
                ...
                signature_algorithm=SignatureMethod.RSA_SHA256,
                digest_algorithm=DigestAlgorithm.SHA256).sign(
                soap_etree, key=self.key, cert=self.cert, key_info=key_info,
                reference_uri=["#_0", "#_1"],
